...
export class HeadNavStoreService {
	...
	readonly completedHeadNavs$ = this.headNav$.pipe(map(headNav => headNav.filter(headnav => headnav.isCompleted)));

	readonly lastHeadNavs$ = this.headNav$.pipe(map(headNav => [headNav[headNav.length - 1]]));
	...
	add(title: _title) {
		...
		this.headNav = [...this.headNav, { id: this.headNav.length + 1, title, isCompleted: false }];
	}
