...
export class UserStore {
  ...
  readonly userIsUser$ = this.user$.pipe(
    map(user => user.filter(user => user.getRole()==Role.USER))
  )

  readonly userIsAdmin$ = this.user$.pipe(
    map(user => user.filter(user => user.getRole()==Role.ADMIN))
  )

  readonly last_users$ = this.user$.pipe(
    map(user => [user[user.length - 1]])
  )
