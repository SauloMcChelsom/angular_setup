...
import { OpenSnackBarService } from '@app/shared/services/open_snack_bar/open_snack_bar.service';
import { HttpErrorResponseService } from '@app/shared/services/http_error/http_error_response.service';
...
export class LoginComponent implements OnInit {
	...
	private createForm(form: Forms) {
		this.form = new FormGroup({
			user: new FormControl({ value: form.user, disabled: false }, Validators.compose([Validators.required])),
			password: new FormControl({ value: form.password, disabled: false }, Validators.compose([Validators.required])),
		});
	}
	...
	public validateForm() {
		...
		if (this.user.value.length < this.MIN_CHAR_USER) {
			this.open.error(
				`O nome do usuário contém ${this.MIN_CHAR_USER} a ${this.MAX_CHAR_USER} caracteres, faltam ${this.MIN_CHAR_USER - this.user.value.length}`
			);
			this.loading = false;
			this.user.setErrors({ incorrect: true });
			this.user.markAsTouched();
		...
		if (this.user.value.length > this.MAX_CHAR_USER) {
			this.open.error(
				`O nome do usuário contém ${this.MIN_CHAR_USER} a ${this.MAX_CHAR_USER} caracteres, remova ${this.user.value.length - this.MAX_CHAR_USER}`
			);
			this.loading = false;
			this.user.setErrors({ incorrect: true });
			this.user.markAsTouched();
		...
		if (this.password.value.length < this.MIN_CHAR_PASSWORD) {
			this.open.error(
				`A senha do usuário contém ${this.MIN_CHAR_PASSWORD} a ${this.MAX_CHAR_PASSWORD} caracteres, faltam ${
					this.MIN_CHAR_PASSWORD - this.password.value.length
				}`
			);
			this.loading = false;
			this.password.setErrors({ incorrect: true });
		...
		if (this.password.value.length > this.MAX_CHAR_PASSWORD) {
			this.open.error(
				`A senha do usuário contém ${this.MIN_CHAR_PASSWORD} a ${this.MAX_CHAR_PASSWORD} caracteres, remova ${
					this.password.value.length - this.MAX_CHAR_PASSWORD
				}`
			);
			this.loading = false;
			this.password.setErrors({ incorrect: true });
