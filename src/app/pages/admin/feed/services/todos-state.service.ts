...
export class TodosStateService {
	...
	public todosDone$: Observable<Todo[]> = this.store$
		.select(appSelectors.Items)
		.pipe(map(todos => (todos == null ? null : todos.filter(todo => todo.isDone))));

	public todosNotDone$: Observable<Todo[]> = this.store$
		.select(appSelectors.Items)
		.pipe(map(todos => (todos == null ? null : todos.filter(todo => !todo.isDone))));
	...
	constructor(
		...
		private apiService: TodosApiService
	) {
		this.load();
	...
	public updateFilter(filter: Filter) {
		...
		if (filter.category.isPrivate && filter.category.isBusiness == false) {
			Private$.pipe(map(todos => (todos == null ? null : todos.filter(todo => todo.isDone))))
				.subscribe((r: Todo[]) => {
					this.state$.next(r);
				})
				.unsubscribe();
		...
		if (filter.category.isBusiness && filter.category.isPrivate == false) {
			Business$.pipe(map(todos => (todos == null ? null : todos.filter(todo => todo.isDone))))
				.subscribe((r: Todo[]) => {
					this.state$.next(r);
				})
				.unsubscribe();
